...
void califica(const Lista<string>& bien, const Lista<string>& mal, Lista<Puntuacion>& listado) {
	...
	while (iBien != finBien) {
		iDi = alumnos.busca(iBien.elem());
		if (iDi != alumnos.end()) iDi.setVal(iDi.valor() + 1);
		else alumnos.inserta(iBien.elem(), 1);
		iBien.next();
	}

	while (iMal != finMal) {
		iDi = alumnos.busca(iMal.elem());
		if (iDi != alumnos.end()) iDi.setVal(iDi.valor() - 1);
		else alumnos.inserta(iMal.elem(), -1);
		iMal.next();
	}
