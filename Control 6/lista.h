...
public:
	...
	void enredar() {
		...
		Nodo *aux, *sigPrincipio, *sigFinal;
		Nodo *primero = _prim;
		Nodo *ultimo = _ult;
		aux = _ult;
		for (int i = 0; i < (longitud() / 2); i++) {
			sigPrincipio = primero->_sig;		// Avanza por delante
			sigFinal = ultimo->_ant;			// Avanza por detras

			primero->_ant = ultimo->_sig;
			primero->_sig = ultimo;

			ultimo->_ant = primero;
			ultimo->_sig = sigPrincipio;

			primero = sigPrincipio;
			aux = ultimo;
			ultimo = sigFinal;
		}
		if (primero == ultimo) {		// Cuando la longitud es impar, acaban en el mismo nodo
			_ult = ultimo;
			ultimo->_sig = nullptr;
		}
		else {							// Cuando es par, el ultimo se pasa
			_ult = aux;
			aux->_sig = nullptr;
		}
	}
